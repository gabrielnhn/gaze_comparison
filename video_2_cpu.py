        ...
        while retval:
            ...
            if faces is not None: 
                for box, landmarks, score in faces:
                    ...
                    draw_gaze(x_min,y_min,bbox_width, bbox_height,frame,(yaw_predicted,pitch_predicted),color=(185, 255, 113), scale=1, thickness=2, size=x_max-x_min, bbox=((x_min, y_min), (x_max, y_max)))
                    
                    yaw,pitch = model.forward(img)
                    yaw = list(yaw.cpu().detach().numpy()[0])
                    pitch = list(pitch.cpu().detach().numpy()[0])

                    y_idx = yaw.index(max(yaw))
                    p_idx = pitch.index(max(pitch))
                    y = y_idx * model.binwidth - 180
                    p = p_idx * model.binwidth - 180

                    yaw_predicted= y * np.pi/180.0
                    pitch_predicted= p * np.pi/180.0
                    draw_gaze(x_min,y_min,bbox_width, bbox_height,frame,(yaw_predicted,pitch_predicted),color=(105, 140, 255), scale=1, thickness=2, size=x_max-x_min, bbox=((x_min, y_min), (x_max, y_max)))
