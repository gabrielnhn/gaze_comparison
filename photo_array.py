...
if __name__ == '__main__':
    ...
    colors = [random_color() for _ in range(model.num_bins)]
        ...
        for frame, filename in frames:
            ...
            if faces is not None: 
                for box, landmarks, score in faces:
                    ...
                    yaw = yaw.cpu().detach().numpy()[0]
                    pitch = pitch.cpu().detach().numpy()[0]
                    ...
                    for i in range(len(yaw)):
                        for j in range(len(pitch)):
                            ...
                            y_len = yaw[i]
                            p_len = pitch[j]
                            y = i * model.binwidth - 180
                            p = j * model.binwidth - 180

                            yaw_predicted= y * np.pi/180.0
                            pitch_predicted= p * np.pi/180.0
                        
                            scale = int(y_len * p_len * 25)
                            ...
                            print(yaw_predicted, pitch_predicted)
                            draw_gaze(x_min,y_min,bbox_width, bbox_height,frame,(yaw_predicted,pitch_predicted),color=colors[i], scale=scale, thickness=1, size=x_max-x_min, bbox=((x_min, y_min), (x_max, y_max)), tip=False)
                    
                    #  cv2.putText(frame, f"{pitch_predicted, yaw_predicted}", (x_min,y_min), cv2.FONT_HERSHEY_PLAIN, 2, (255, 255, 255), 2)

            cv2.imwrite("gaze_array_"+filename, frame)